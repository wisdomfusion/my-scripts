#!/usr/bin/env bash
# build_deploy_express.sh
# Build Script for Node.js/Express.js Application
...
# -h 可选，部署主机列表
# eg. ./build_deploy_express.sh -n "med-selection-web" -b main -r build -e prod -h "172.17.212.190"
...
echo "Installing app server dependencies using pnpm..."
if ! pnpm --registry https://registry.npmmirror.com install; then
    log_error "Failed to install dependencies"
    exit 1
fi

echo "Changing directory to ${APP_SRC_DIR}app_assets/..."
cd ${APP_SRC_DIR}app_assets/ || {
    log_error "Failed to change directory to ${APP_SRC_DIR}app_assets/"
    exit 1
}

echo "Installing app assets dependencies using pnpm..."
if ! pnpm --registry https://registry.npmmirror.com install; then
    log_error "Failed to install dependencies"
    exit 1
fi
...
if ! pnpm $NPM_RUN; then
    log_error "Failed to build app assets"
    exit 1
fi
